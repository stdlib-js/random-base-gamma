...
tape( 'attached to the main export is a method to generate pseudorandom number generators', function test( t ) {
	t.strictEqual( typeof gamma.factory, 'function', 'has method' );
	t.end();
});
...
tape( 'the function supports setting the generator state (alpha >= 1.0)', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( gamma( 2.0, 2.0 ), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
...
tape( 'the function supports setting the generator state (alpha < 1.0)', function test( t ) {
	...
	// Replay previously generated values...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( gamma( 0.05, 2.0 ), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
