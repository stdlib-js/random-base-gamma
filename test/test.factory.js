...
tape( 'attached to the returned function is the generator name', function test( t ) {
	var gamma = factory();
	t.strictEqual( gamma.NAME, 'gamma', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var gamma = factory();
	t.strictEqual( typeof gamma.PRNG, 'function', 'has property' );

	gamma = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( gamma.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( gamma.seed ), true, 'has property' );
	t.strictEqual( gamma.seed[ 0 ], 12345, 'equal to provided seed' );

	gamma = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( gamma.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var gamma = factory();
	t.strictEqual( typeof gamma.seedLength, 'number', 'has property' );

	gamma = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( gamma.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the return function is the generator state', function test( t ) {
	var gamma = factory();
	t.strictEqual( isUint32Array( gamma.state ), true, 'has property' );

	gamma = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( gamma.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var gamma = factory();
	t.strictEqual( typeof gamma.stateLength, 'number', 'has property' );

	gamma = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( gamma.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var gamma = factory();
	t.strictEqual( typeof gamma.byteLength, 'number', 'has property' );

	gamma = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( gamma.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	gamma = factory();
	t.strictEqual( typeof gamma.toJSON, 'function', 'has method' );

	o = gamma.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, gamma.NAME, 'has property' );
	t.deepEqual( o.state, typedarray2json( gamma.state ), 'has property' );
	t.deepEqual( o.params, [], 'has property' );

	gamma = factory( 1.0, 2.0 );
	o = gamma.toJSON();
	...
	t.strictEqual( typeof gamma.toJSON, 'function', 'has method' );
	t.strictEqual( gamma.toJSON(), null, 'returns expected value' );
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( gamma(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
...
tape( 'the returned function supports setting the generator state (alpha < 1.0)', function test( t ) {
	var gamma;
	var state;
	var arr;
	var i;

	gamma = factory( 0.2, 4.0 );
